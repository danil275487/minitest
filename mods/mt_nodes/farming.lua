...
function farming:place_seed(itemstack, placer, pointed_thing, plantname)
	...
	if string.find(farmland.name, "mt_nodes:farmland") and string.find(place_s.name, "air")  then
...
--Farmlands
minetest.register_node("mt_nodes:farmland", {
...
minetest.register_node("mt_nodes:farmland_wet", {
	description = "Farmland",
...
minetest.register_abm({
	...
	nodenames = {"mt_nodes:farmland", "mt_nodes:farmland_wet"},
	interval = 15,
	chance = 4,
	action = function(pos, node)
		-- Get wetness value
		...
		if not wet then
			if node.name == "mt_nodes:farmland" then
				wet = 0
			else
				wet = 7
			end
		...
		if check_surroundings(pos, "group:water") then
			if node.name ~= "mt_nodes:farmland_wet" then
				-- Make it wet
				node.name = "mt_nodes:farmland_wet"
				minetest.set_node(pos, node)
			end
...
for i = 1, 3 do
	minetest.register_node("mt_nodes:wheat_plant_"..i, {
		description = "Wheat",
		paramtype = "light",
...
minetest.register_node("mt_nodes:wheat_plant", {
	description = "Wheat",
	sunlight_propagates = true,
	paramtype = "light",
	paramtype2 = "meshoptions",
	...
	drop = {
		max_items = 4,
		items = {
			{ items = {'mt_items:seeds'} },
			{ items = {'mt_items:seeds'}, rarity = 2},
			{ items = {'mt_items:seeds'}, rarity = 5},
			{ items = {'mt_items:wheat'} }
		}
	},
